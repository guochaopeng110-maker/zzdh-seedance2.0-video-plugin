...
- **Global scope**: `F:/Projects/zz-video-plugins/.gemini/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `F:/Projects/zz-video-plugins/.gemini/notes/`)
...
2. Glob `F:/Projects/zz-video-plugins/.gemini/notes/*.md` (if directory exists) — global notes
...
Global (F:/Projects/zz-video-plugins/.gemini/notes/):
...
2. **No `.planning/`**: Falls back to global `F:/Projects/zz-video-plugins/.gemini/notes/` — works in any directory
