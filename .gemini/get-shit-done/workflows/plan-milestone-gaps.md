...
HIGHEST=$(node "F:/Projects/zz-video-plugins/.gemini/get-shit-done/bin/gsd-tools.cjs" phases list --pick directories[-1])
```
...
```bash
node "F:/Projects/zz-video-plugins/.gemini/get-shit-done/bin/gsd-tools.cjs" commit "docs(roadmap): add gap closure phases {N}-{M}" --files .planning/ROADMAP.md .planning/REQUIREMENTS.md
```
